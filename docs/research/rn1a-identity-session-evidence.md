...
This record supports [RN1A](https://github.com/dornglut/runen-net/issues/4). It compares current Runenwerk migration evidence with external networking designs. It does not define RunenNet semantics.
...
Runenwerk `net/engine_sim/src/identity.rs` currently colocates `SimulationTick`, `SimulationSessionId`, `ActorId`, and `NetEntityId`. `SimulationTick` and `SimulationSessionId` derive concrete `ecs::Component` and `ecs::Resource`, and `SimulationSessionId::new()` allocates from a process-global `AtomicU64`.
...
Source: [Runenwerk simulation identity](https://github.com/dornglut/runenwerk/blob/main/net/engine_sim/src/identity.rs).
...
Source: [Runenwerk session state](https://github.com/dornglut/runenwerk/blob/main/net/engine_net/src/session/ids.rs).
...
- [Runenwerk admission](https://github.com/dornglut/runenwerk/blob/main/net/engine_net/src/session/admission.rs)
- [Runenwerk client handoff](https://github.com/dornglut/runenwerk/blob/main/net/engine_net/src/session/handoff.rs)

### Admission also carries game-specific state

`AuthoritativeJoinState` currently contains lobby IDs, roster player codes, player/AI limits, and JSON settings. Those are application/game concerns, not general session identity semantics, and should not migrate into the standalone core.
...
Source: [Runenwerk reconnect and history recovery design](https://github.com/dornglut/runenwerk/blob/main/docs-site/src/content/docs/design/active/net-reconnect-history-recovery.md).
...
Current Lightyear documentation describes a connection layer with a long-running `PeerId` over lower-level links. This is not authority for RunenNet, but it demonstrates that a stable application-network participant identity can remain useful independently of the transport link that currently carries traffic.

Source: [Lightyear current crate documentation](https://docs.rs/lightyear/latest/lightyear/).

### Secure connection protocols also separate client identity from endpoint mapping

The netcode standard carries a client ID in authenticated connection-token data while maintaining separate endpoint/encryption mappings and packet sequence/replay state. This again illustrates that client/application identity need not be the network address or transient connection realization.
...
2. **Participant identity** is session-scoped and is the stable identity for an admitted remote participant. It may survive replacement of the transport connection within the same session.
...
4. **Network entity identity** is authority-owned and session-scoped. Reuse during the same session should be avoided so delayed traffic cannot target a different entity incarnation.
...
6. **Separate authority epoch** is not justified for the initial core if each distinct authoritative session lifetime has a non-reused `SessionId`. A later failover model that preserves one logical session across authority replacement may add an authority-generation concept explicitly.
...
The normative slice still has to decide:

- the required uniqueness/non-reuse horizon for session identifiers without freezing a wire width or generation algorithm;
- whether the initial client/server profile permits only one current connection binding per participant;
- exactly which membership/session transitions RN2 must implement;
- which lifecycle effects are required on connection loss versus left open for later reconnect/recovery profiles.

Exact integer widths, serialization, cryptographic generation, public Rust type names, and QUIC mapping are outside RN1A.
